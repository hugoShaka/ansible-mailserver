...
creating an alias per user. Domain aliases are defined in the
``virtual_alias_domains`` database table. Domain-wide aliasing is done in 2
steps :
...
2. Creating the domain-wide alias. This is done by the ``virtual_alias_maps``
   ...
   destination domain. The postgresql query is described in
